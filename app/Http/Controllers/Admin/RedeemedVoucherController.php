...
class RedeemVoucherController extends Controller {
    ...
    {
        $redeemvoucher = RedeemVoucher::where('redeem_by' , Auth::user()->id)->with('customerDetails','voucherDetails')->get();
	...
	public function getVoucherDetails(Request $request)
	{
		...
		if($voucherDetails != null){
			...
			// return view('admin.redeemvoucher.details' , compact('voucherDetails', 'customerId'));
			// return back()->withErrors(array('message' => 'This voucher is not valid.'));
	}
	/**
	 * Show the form for creating a new redeemvoucher
	 *
     * @return \Illuminate\View\View
	 */
	...
	/**
		* Store a newly created redeemvoucher in storage.
		*
		* @param CreateRedeemVoucherRequest|Request $request
	*/
    			...
    			if( !$alreadyRedeem ){
    				...
    				if( $redeemed ){
    					...
    					\Mail::send(['text'=>'mail'], $data, function($message) {
    						$message->to('discountindia98@gmail.com', 'Discount India')->subject('Voucher Redeemed');
	...
	/**
	 * Show the form for editing the specified redeemvoucher.
	 *
	 * @param  int  $id
     * @return \Illuminate\View\View
	 */
	...
	/**
	 * Update the specified redeemvoucher in storage.
     * @param UpdateRedeemVoucherRequest|Request $request
     *
	 * @param  int  $id
	 */
	...
	/**
	 * Remove the specified redeemvoucher from storage.
	 *
	 * @param  int  $id
	 */
