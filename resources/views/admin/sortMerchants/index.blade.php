...
<p>{!! link_to_route(config('coreadmin.route').'.hotdeals.create', trans('coreadmin::templates.templates-view_index-add_new') , null, array('class' => 'btn btn-success')) !!}</p>

@if ($listVoucher->count())
    <div class="portlet box green">
        ...
        <div class="portlet-body">
            <table class="table table-striped table-hover table-responsive ">
                ...
                <tbody id="sortable">
                    @foreach ($listVoucher as $k => $row)
                        <tr data-id="{{ $row->id }}" data-order="{{ $row->order }}">
                            
                            <td>{{ $row->title }}</td>
                            <td>{{ $row->voucher_unique_id }}</td>
                            {{-- <td>
                                {!! link_to_route(config('coreadmin.route').'.hotdeals.edit', trans('coreadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-xs btn-info')) !!}
                                {!! Form::open(array('style' => 'display: inline-block;', 'method' => 'DELETE', 'onsubmit' => "return confirm('".trans("coreadmin::templates.templates-view_index-are_you_sure")."');",  'route' => array(config('coreadmin.route').'.hotdeals.destroy', $row->id))) !!}
            ...
            {!! Form::open(['route' => config('coreadmin.route').'.hotdeals.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
        ...
        $(document).ready(function () {
            ...
            $( "#sortable" ).sortable({
                update: function( event, ui ) {
                    ...
                    $.ajax({
                        url: "{{ route('sort.merchant.voucher') }}",
                        type:"POST",
