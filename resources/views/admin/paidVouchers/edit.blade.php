...
{!! Form::model($paidvouchers ,array('route' =>  'update.paid.voucher', 'id' => 'form-with-validation', 'class' => 'form-horizontal','autocomplete' => 'off' , "enctype" => 'multipart/form-data')) !!}
{{-- {!! Form::model($paidvouchers ,array('route' => 'craete.paid.voucher', 'id' => 'form-with-validation', 'class' => 'form-horizontal','autocomplete' => 'off' , "enctype" => 'multipart/form-data')) !!} --}}
    <input type="hidden" name="selectedVoucherId" value="{{ $paidvouchers->id }}">
    <div class="form-group">
        {!! Form::label('voucher_template', 'Voucher Template*', array('class'=>'col-sm-2 control-label')) !!}
        <div class="col-sm-10">
            <div class="form-check">
                <label class="form-check-label">
                    <input type="radio" class="form-check-input" name="voucherType" {{ ($paidvouchers->fileName != null && $paidvouchers->fileName != '') ? 'checked' : '' }} value="uploadVoucher">Upload voucher
                </label>
            </div>
            <div class="form-check">
                <label class="form-check-label">
                    <input type="radio" class="form-check-input" name="voucherType" {{ ($paidvouchers->fileName == null && $paidvouchers->fileName == '') ? 'checked' : '' }} value="preTemp">Pre Template
                </label>
            </div>
        </div>
    </div>
    <div class="voucherslistimages {{ ($paidvouchers->fileName == null && $paidvouchers->fileName == '') ? 'show' : 'hide' }}" >
        <div class="form-group " >
            {!! Form::label('voucher_template', 'Voucher Template*', array('class'=>'col-sm-2 control-label')) !!}
            <div class="col-sm-10">
    ...
    <div class="browseImage {{ ($paidvouchers->fileName != null && $paidvouchers->fileName != '') ? 'show' : 'hide' }}" >
        <div class="form-group">
            {!! Form::label('voucher_template', 'Browse Image', array('class'=>'col-sm-2 control-label')) !!}

            <div class="col-sm-10">
                <div class="custom-file">
                    <input type="file" class="form-control custom-file-input" name="bannerImage" id="inputGroupFile01">
                    <small style="color: red">( Stay empty if you dont want to change image )</small>
                    <br>
                    <img src="{{ asset('voucher/images/'.$paidvouchers->fileName) }}" style="width: 200px">
                </div>
            </div>
        </div>
    </div>
    <div class="form-group">
        {!! Form::label('voucher_template', 'Vouchers*', array('class'=>'col-sm-2 control-label')) !!}
        <div class="col-sm-10">
            <select class="selectpicker" data-show-subtext="true" data-live-search="true" name="voucher_of">
                @foreach( $vouchers as $k => $v )
                    {{-- <option data-subtext="{{ $v->voucher_unique_id }} ({{ $v->title }})" value="{{ $v->id }}">{{ $v->voucher_unique_id }} ({{ $v->title }})</option> --}}
                    @if( $v->Offer != null )
                        <option {{ ($paidvouchers->voucher_of == $v->offer->id)? 'selected' : '' }} data-subtext="{{ ($v->Offer != null) ? $v->Offer->title : '' }} ({{ (strlen($v->title) > 50 ) ? substr($v->title, 0 , 50).' .....' : $v->title }})" value="{{ $v->Offer->id }}">{{ $v->voucher_unique_id }}</option>
                    @endif
    ...
    <div class="form-group">
        {!! Form::label('customer_price', 'Customer Price*', array('class'=>'col-sm-2 control-label')) !!}
        <div class="col-sm-10">
            {!! Form::text('customer_price', old('customer_price'), array('class'=>'form-control')) !!}
            
        </div>
    </div>
    <div class="form-group">
        {!! Form::label('hotel_price', 'Hotel Price*', array('class'=>'col-sm-2 control-label')) !!}
        <div class="col-sm-10">
            {!! Form::text('hotel_price', old('hotel_price'), array('class'=>'form-control')) !!}
...
@section('javascript')
    ...
    jQuery(document).ready(function(e){
        ...
        $(document).on('change' , 'input[name=voucherType]' , function(){
            if( $(this).val() == "preTemp" ){
                $('.voucherslistimages').addClass('show');
                $('.browseImage').addClass('hide');
            }
