...
{!! Form::open(array('route' => 'craete.paid.voucher', 'id' => 'form-with-validation', 'class' => 'form-horizontal','autocomplete' => 'off' , "enctype" => 'multipart/form-data')) !!}
        ...
        <div class="col-sm-10">
            <select class="selectpicker" data-show-subtext="true" data-live-search="true" name="voucher_of">
                @foreach( $vouchers as $k => $v )
                    ...
                    @if( $v->Offer != null )
	                    <option data-subtext="{{ ($v->Offer != null) ? $v->Offer->title : '' }} ({{ (strlen($v->title) > 50 ) ? substr($v->title, 0 , 50).' .....' : $v->title }})" value="{{ $v->Offer->id }}">{{ $v->voucher_unique_id }}</option>
                    @endif
